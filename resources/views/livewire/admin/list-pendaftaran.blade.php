                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <small class="text-gray-500">
                                        {{ is_array($p->program->penyelenggara)
                                            ? implode(', ', $p->program->penyelenggara)
                                            : $p->program->penyelenggara
                                        }}
                                    </small>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-center">
                                    ...
                                    <span
                                        class="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm">Disetujui</span>
                                    @elseif($p->status === 'finished')
                                    <span
                                        class="bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm">Selesai</span>
                                    @else
                                    <span
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center gap-3">
                                        ...
                                        <button wire:click="updateStatus({{ $p->id }}, 'approve')"
                                            class="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-lg text-sm">
                                            Setujui
                                        </button>
                                        ...
                                        {{-- STATUS APPROVED --}}
                                        @elseif ($p->status === 'approved')
                                        {{-- Jika program offline  → wajib kirim perizinan --}}
                                        @if (strtolower($p->program->pelaksanaan) === 'offline')
                                        <a href="{{ route('create-perizinan', $p->id) }}"
