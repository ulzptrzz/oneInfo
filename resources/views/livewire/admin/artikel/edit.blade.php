                ...
                <form wire:submit.prevent="update" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea wire:model="deskripsi" rows="5"
                            class="w-full border-2 border-gray-200 rounded-lg px-4 py-3 
                               focus:border-[#FFC436] focus:outline-none transition resize-none"></textarea>
                        @error('deskripsi')
                            <p class="text-red-600 text-sm mt-1 flex items-center gap-1">
                                <i class='bx bx-error-circle'></i> {{ $message }}
                            </p>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Thumbnail Baru</label>

                        <div class="flex items-center gap-4">
                            <input type="file" wire:model="thumbnail" accept="image/*" id="thumbInputEdit"
                                class="hidden">

                            <button type="button" onclick="document.getElementById('thumbInputEdit').click()"
                                class="bg-[#FFC436] text-[#0C356A] px-6 py-3 rounded-lg font-semibold 
                                   hover:bg-yellow-400 transition">
                                Choose File
                            </button>

                            {{-- If no new thumbnail yet --}}
                            @if (!$thumbnail)
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-500">
                                                {{ number_format($thumbnail->getSize() / 1024, 2) }} KB</p>
                                        </div>
                                    </div>
                    ...
                    {{-- Thumbnail Lama --}}
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Thumbnail Lama</label>
                        <img src="{{ asset('storage/' . $thumbnail_lama) }}"
                            class="h-32 object-cover rounded-lg shadow border">
                    </div>

                    {{-- Action Buttons --}}
                    <div class="flex flex-col sm:flex-row justify-end gap-3 pt-6 border-t border-gray-200">
                        <a href="{{ route('admin.artikel') }}"
                            class="px-6 py-3 border-2 border-gray-300 text-gray-700 font-semibold rounded-lg 
